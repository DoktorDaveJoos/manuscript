...
export default function Export({ book, storylines }: Props) {
    ...
    const [storylineId, setStorylineId] = useState<number | null>(
    ...
    const handleExport = useCallback(() => {
        ...
        if (scope === 'storyline' && storylineId)
            data.storyline_id = storylineId;
    ...
    return (
        <SettingsLayout
            ...
            book={book}
            title={t('export.pageTitle', { bookTitle: book.title })}
        >
                    ...
                    <div>
                        ...
                        {scope === 'storyline' && storylines.length > 1 && (
                            <Select
                                value={storylineId ?? ''}
                                onChange={(e) =>
                                    setStorylineId(Number(e.target.value))
                                }
                        ...
                        <div className="mt-3 flex flex-col">
                            <div className="flex items-center justify-between border-b border-border-light py-3">
                                <span className="text-[14px] text-ink">
                                    {t('export.includeChapterTitles')}
                                </span>
                                <Toggle
                                    checked={includeChapterTitles}
                                    onChange={() =>
                                        setIncludeChapterTitles(
                                            !includeChapterTitles,
                                        )
                                    }
                                />
                            ...
                            <div className="flex items-center justify-between py-3">
                                <span className="text-[14px] text-ink">
                                    {t('export.includeActBreaks')}
                                </span>
                                <Toggle
                                    checked={includeActBreaks}
                                    onChange={() =>
                                        setIncludeActBreaks(!includeActBreaks)
                                    }
                                />
                            </div>
                        </div>
