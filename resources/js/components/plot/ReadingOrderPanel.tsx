        ...
        >
            ...
            <span className="shrink-0 truncate text-[11px] text-ink-muted">
                {t('readingOrder.storylineChapter', {
                    storyline:
                        storyline?.name ?? t('readingOrder.unknownStoryline'),
...
export default function ReadingOrderPanel({
    ...
    const handleDragEnd = useCallback(
        (event: DragEndEvent) => {
            ...
            const oldIndex = orderedChapters.findIndex(
                (ch) => `reading-${ch.id}` === active.id,
            );
            const newIndex = orderedChapters.findIndex(
                (ch) => `reading-${ch.id}` === over.id,
            );
    ...
    const activeChapterStoryline = activeChapter
        ...
        >
            ...
            <div className="flex h-12 items-center border-b border-border-light px-4">
                <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-neutral-bg px-1.5 py-0.5 text-[10px] font-medium text-ink-soft tabular-nums">
                        {t('readingOrder.chapterCount', {
            ...
            {hasMultipleStorylines && (
                <div className="flex items-center justify-between border-b border-border-light px-4 py-2">
                    ...
                    >
                        ...
                        {exporting
                            ? t('readingOrder.exporting')
                            : t('readingOrder.export')}
                    </button>
                </div>
            )}
                            ...
                            <div className="flex flex-col gap-px">
                                {mainChapters.map((chapter) => (
                                    <SortableChapterRow
                                        ...
                                        chapter={chapter}
                                        position={
                                            positionMap.get(chapter.id) ?? 0
                                        }
                                        storyline={storylineMap.get(
                                            chapter.storyline_id,
                                        )}
                                        storylineChapterIndex={getStorylineChapterIndex(
                                            chapter,
                                        )}
                                        t={t}
                                    />
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
                    <div className="mx-4 w-full max-w-sm rounded-lg bg-surface-card p-5 shadow-lg">
                        <h3 className="text-[14px] font-semibold text-ink">
                            {t('readingOrder.confirmTitle')}
                        </h3>
                        <p className="mt-2 text-[13px] leading-relaxed text-ink-soft">
                            {t('readingOrder.confirmMessage', {
                                count: orderedChapters.length,
                            })}
                        </p>
