    ...
    return (
        ...
        >
            <div className="flex items-center gap-2">
                ...
                />
                <span className="flex-1 truncate text-[13px] font-medium text-ink">{plotPoint.title}</span>
                <span
            ...
            {plotPoint.description && (
                <p className="mt-1 truncate pl-4 text-[12px] text-ink-muted">{plotPoint.description}</p>
            )}
            {storyline && <p className="mt-0.5 pl-4 text-[11px] text-ink-faint">{storyline.name}</p>}
        </button>
    );
}
        ...
        <div className="flex flex-col gap-6">
            ...
            {unassigned.length > 0 && (
                <div className="flex flex-col gap-2">
                    <h3 className="text-[13px] font-semibold text-ink-muted">{t('unassigned')}</h3>
                    <div className="flex flex-col gap-1.5">
                        {unassigned.map((pp) => (
                            <PlotPointRow
                                key={pp.id}
