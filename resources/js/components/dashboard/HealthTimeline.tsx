...
type MetricKey = 'hooks' | 'pacing' | 'tension' | 'weave' | 'scene_purpose' | 'tension_dynamics' | 'emotional_arc' | 'craft';
...
export default function HealthTimeline({ history }: { history: HealthSnapshot[] }) {
    ...
    const data = useMemo(() => {
        ...
        const metrics: { key: MetricKey }[] = hasNewMetrics
            ? [{ key: 'scene_purpose' }, { key: 'pacing' }, { key: 'tension_dynamics' }, { key: 'hooks' }, { key: 'emotional_arc' }, { key: 'craft' }]
            : [{ key: 'hooks' }, { key: 'pacing' }, { key: 'tension' }, { key: 'weave' }];

        const xScale = (i: number) => history.length === 1
            ? PAD.left + INNER_W / 2
    ...
    return (
        ...
        >
            ...
            {/* Composite area fill */}
            <path d={toAreaPath(data.compositePoints)} className="fill-accent/5" />

            {/* Metric lines (thin, muted) */}
            {data.metricLines.map((m) =>
                m.points.length === 1 ? (
                    <circle key={m.key} cx={m.points[0].x} cy={m.points[0].y} r={2.5} className="fill-ink-faint/40" />
                ) : (
                    <path
                        key={m.key}
                        d={toPath(m.points)}
            ...
            <path d={toPath(data.compositePoints)} fill="none" className="stroke-accent" strokeWidth={2.5} />

            {/* Data point dots */}
            {(data.compositePoints.length === 1
                ? data.compositePoints
                : [data.compositePoints[0], data.compositePoints[Math.floor(data.compositePoints.length / 2)], data.compositePoints[data.compositePoints.length - 1]]
            ).map((p, i) => (
