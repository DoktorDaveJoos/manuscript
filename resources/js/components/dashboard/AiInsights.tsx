...
function ScoreGauge({ score, qualityLabel }: { score: number; qualityLabel: { good: string; fair: string; needsWork: string } }) {
    const size = 96;
    const strokeWidth = 6;
    ...
    return (
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="shrink-0">
            ...
            >
                {score >= 70 ? qualityLabel.good : score >= 50 ? qualityLabel.fair : qualityLabel.needsWork}
            </text>
        </svg>
    );
}
...
function MetricBar({ label, score }: { label: string; score: number }) {
    return (
        <div className="flex items-center gap-2">
            ...
            <span className="w-6 text-right text-[12px] font-semibold text-ink-soft">{score}</span>
        </div>
    );
}
            ...
            <div className="flex flex-1 flex-col gap-4">
                ...
                <ScoreGauge
                    ...
                    qualityLabel={{
                        ...
                        needsWork: t('aiInsights.quality.needsWork', 'Needs Work'),
                    }}
                />

                <div className="flex flex-col gap-[10px]">
                    {healthMetrics.metrics.map((metric) => (
                        <MetricBar key={metric.label} label={t('aiInsights.metrics.' + metric.label, metric.label)} score={metric.score} />
                    ))}
                </div>
            </div>
                ...
                <div className="flex flex-col gap-3">
                    {healthMetrics.attention_items.map((item, i) => {
                        ...
                        if (item.description_key) {
                            title = t('aiInsights.attention.chapterTitle', { order: item.chapter_order, title: item.chapter_title });
                            const params = { ...(item.description_params ?? {}) };
                            if (item.description_key === 'weakHook' && params.type) {
                                params.type = t('aiInsights.hookType.' + params.type, String(params.type));
                            }
                            if (item.description_key === 'informationDelivery' && params.type) {
                                params.type = t('aiInsights.infoDelivery.' + params.type, String(params.type));
                            }
                            description = t('aiInsights.attention.' + item.description_key, params);
                        } else {
                            title = item.title ?? '';
