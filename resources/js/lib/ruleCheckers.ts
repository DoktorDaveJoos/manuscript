...
function findMatches(text: string, pattern: RegExp, maxExamples = 2): RuleResult {
    const matches = text.match(pattern) ?? [];
    return {
        count: matches.length,
        examples: [...new Set(matches.map((m) => m.trim()))].slice(0, maxExamples),
    };
}
...
const WORDY_PHRASES = /\b(in order to|the fact that|at this point in time|due to the fact|for the purpose of|in the event that)\b/gi;

const TELLING_VERBS = /\b(felt that|was feeling|realized that|knew that|thought that|understood that)\b/gi;
...
export const ruleCheckers: Record<string, (text: string) => RuleResult> = {
    ...
    sentence_variety: (text) => {
        ...
        return {
            ...
            examples: lowVariety ? [`Std dev: ${stdDev.toFixed(1)} words (low variety)`] : [],
        };
    },
};
