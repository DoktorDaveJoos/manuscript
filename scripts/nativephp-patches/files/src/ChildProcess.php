...
class ChildProcess implements ChildProcessContract
{
    ...
    {
        if (! is_array($process)) {
            throw new \RuntimeException('NativePHP child process returned an invalid startup response.');
        }

        if (isset($process['error'])) {
            throw new \RuntimeException(
                'NativePHP child process failed to start: '.$process['error'],
            );
        }

        $settings = $process['settings'] ?? null;

        if (! is_array($settings)) {
            throw new \RuntimeException('NativePHP child process response is missing its settings.');
        }
