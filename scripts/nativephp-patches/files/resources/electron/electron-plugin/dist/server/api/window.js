...
const windowLoadPromises = new Map();
export function getWindowLoadPromise(id) {
    return windowLoadPromises.get(id) ?? null;
}
...
router.post('/open', (req, res) => {
    ...
    window.on('closed', () => {
        if (windowLoadPromises.get(id) === loadPromise) {
            windowLoadPromises.delete(id);
        }
    });
    window.on('hide', (evt) => {
        notifyLaravel('events', {
    ...
    loadPromise.catch(() => {});
    windowLoadPromises.set(id, loadPromise);
